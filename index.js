...
bot.command("monitoruser", async (bad) => {
...
Total Users: ${Object.keys(usersFile).length}
    ...
    await bad.replyWithMarkdown(message);
});

bot.command('wamenu', async (bad) => {
await bad.deleteMessage();
const Reothermenu = `
┏━━━━━━𝐂͢𝐎͠𝐌𝐌͜𝐀𝐍𝐃━━━━━
...
bot.command('ownermenu', async (bad) => {
await bad.deleteMessage();
const OwnMenu = `
┏━━━━━━𝐂͢𝐎͠𝐌𝐌͜𝐀𝐍𝐃━━━━━
  ☞ /runtime
...
bot.command('toolmenu', async (bad) => {
await bad.deleteMessage();
const BugMenu = `
┏━━━━━━𝐂͢𝐎͠𝐌𝐌͜𝐀𝐍𝐃━━━━━
  ☞ /checkhost
...
Developer : @Kingbadboi`;
    ...
    bot.on('message', async (bad) => {
        require("./XeonBug18Xtele")(bad, bot)
...
require("./XeonBug18.js")(bad, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
